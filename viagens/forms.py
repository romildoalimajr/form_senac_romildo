...
class ViagemForms(forms.Form):
    ...
    data_pesquisa = forms.DateField(label = 'Data de Pessquisa', disabled = True, initial = datetime.today)
    ...
    def clean_origem(self):
        ...
        if any(char.isdigit() for char in origem):
            raise forms.ValidationError('Entrada inválida!')
        else:
            return origem
    '''
    ...
    def clean(self):
        ...
        campo_temNumero(origem, 'destino', listaErros)
        origem_destino_iguais(origem, destino, listaErros)
        compara_data_ida_volta(data_ida, data_volta, listaErros)
        copmpara_data_ida_pesquisa(data_ida, data_pesquisa, listaErros)
